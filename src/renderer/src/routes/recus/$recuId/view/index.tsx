...
type LoaderData = RecuAttributes & {
  ...
  series?: { vignetteValueId: string; seriesStart: string; seriesEnd: string; vignetteValue?: VignetteValueAttributes }[]
}
        ...
        <TabsContent value="details">
          <Card>
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <FeildView
                label="Créé le"
                value={format(new Date(recu.createdAt || recu.dateRecu), 'dd/MM/yyyy', { locale: fr })}
              />
              <FeildView label="Note" className="md:col-span-3" value={(recu as any).note || '--'} />
                    ...
                    <div>
                      {(recu.items || []).map((line) => (
                        <div key={line.id} className="flex items-center justify-between p-3 text-sm">
                          ...
                          <span className="font-medium">{Number(line.subtotalDh).toFixed(2)} DH</span>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {(recu.series || []).length === 0 ? (
                    ...
                    <div>
                      {(recu.series || []).map((s: any, i: number) => (
                        <div key={`${s.vignetteValueId}_${i}`} className="flex items-center justify-between p-3 text-sm">
                          <div className="flex flex-col">
                            <span>
                              {`${(s as any).vignetteValue?.valueDh?.toFixed?.(2) || ''} DH : ${s.seriesStart} → ${s.seriesEnd}`}
                            </span>
                          </div>
                        </div>
                      ))}
